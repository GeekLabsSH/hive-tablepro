...
const useUtilityClasses = (ownerState) => {
  ...
  return compClasses(slots, getSelectUtilityClasses, classes);
};
  ...
  const items = childrenArray.map((child, index, arr) => {
    ...
    if (process.env.NODE_ENV !== null) {
      if (isFragment(child)) {
        console.error(
          [
            "HIVE: The Select component doesn't accept a Fragment as a child.",
    ...
    const isFirstSelectableElement = () => {
      ...
      const firstSelectableElement = arr.find(
        (item) =>
          item?.props?.value !== undefined && item.props.disabled !== true
      );
  ...
  if (process.env.NODE_ENV !== null) {
    ...
    React.useEffect(() => {
      if (!foundMatch && !multiple && value !== "") {
        ...
        console.warn(
          [
            `HIVE: You have provided an out-of-range value \`${value}\` for the select ${
              name ? `(name="${name}") ` : ""
            }component.`,
