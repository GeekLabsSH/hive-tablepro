...
import { refType } from "@cronoslogistics/hive-tablepro/core/mui-utils/src";
...
const useUtilityClasses = (ownerState) => {
  ...
  return compClasses(slots, getTabsUtilityClass, classes);
};
...
const Tabs = React.forwardRef(function Tabs(inProps, ref) {
  ...
  if (process.env.NODE_ENV !== null) {
    if (centered && scrollable) {
      console.error(
        'HIVE: You can not use the `centered={true}` and `variant="scrollable"` properties ' +
      ...
      if (children.length > 0) {
        ...
        if (process.env.NODE_ENV !== null) {
          if (!tab) {
            console.error(
              [
                `HIVE: The \`value\` provided to the Tabs component is invalid.`,
  ...
  const children = React.Children.map(childrenProp, (child) => {
    ...
    if (process.env.NODE_ENV !== null) {
      if (isFragment(child)) {
        console.error(
          [
            "HIVE: The Tabs component doesn't accept a Fragment as a child.",
