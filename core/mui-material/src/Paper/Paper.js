...
const useUtilityClasses = (ownerState) => {
  ...
  return compClasses(slots, getPaperUtilityClass, classes);
};
...
const Paper = React.forwardRef(function Paper(inProps, ref) {
  ...
  if (process.env.NODE_ENV !== null) {
    ...
    if (theme.shadows[elevation] === undefined) {
      console.error(
        [
          `HIVE: The elevation provided <Paper elevation={${elevation}}> is not available in the theme.`,
...
Paper.propTypes /* remove-proptypes */ = {
  ...
  elevation: chainPropTypes(integerPropType, (props) => {
    ...
    if (elevation > 0 && variant === "outlined") {
      return new Error(
        `HIVE: Combining \`elevation={${elevation}}\` with \`variant="${variant}"\` has no effect. Either use \`elevation={0}\` or use a different \`variant\`.`
      );
    }
