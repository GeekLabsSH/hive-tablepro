...
const useUtilityClasses = (ownerState) => {
  ...
  return compClasses(slots, getAvatarGroupUtilityClass, classes);
};
  ...
  const children = React.Children.toArray(childrenProp).filter((child) => {
    if (process.env.NODE_ENV !== null) {
      if (isFragment(child)) {
        console.error(
          [
            "HIVE: The AvatarGroup component doesn't accept a Fragment as a child.",
...
AvatarGroup.propTypes /* remove-proptypes */ = {
  ...
  max: chainPropTypes(PropTypes.number, (props) => {
    if (props.max < 2) {
      return new Error(
        [
          "HIVE: The prop `max` should be equal to 2 or above.",
