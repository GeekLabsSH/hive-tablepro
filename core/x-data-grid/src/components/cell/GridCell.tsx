...
type OwnerState = Pick<
  GridCellProps,
  "align" | "showRightBorder" | "isEditable" | "isSelected"
> & {
  classes?: DataGridProcessedProps["classes"];
};

const useUtilityClasses = (ownerState: OwnerState) => {
  const { align, showRightBorder, isEditable, isSelected, classes } =
    ownerState;
...
function GridCell(props: GridCellProps) {
  ...
  React.useEffect(() => {
    ...
    if (cellRef.current && !cellRef.current.contains(doc.activeElement!)) {
      ...
      const elementToFocus =
        focusElementRef.current || focusableElement || cellRef.current;
  ...
  if (
    ...
    handleFocus = (event: React.FocusEvent) => {
      ...
      if (!warnedOnce) {
        console.warn(
          [
            `HIVE: The cell with id=${rowId} and field=${field} received focus.`,
  ...
  const renderChildren = () => {
    ...
    if (React.isValidElement(children as any) && managesOwnFocus) {
  ...
  return (
    <div
      ...
      style={style}
      tabIndex={
        (cellMode === "view" || !isEditable) && !managesOwnFocus ? tabIndex : -1
      }
