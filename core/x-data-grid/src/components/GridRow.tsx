...
const useUtilityClasses = (ownerState: OwnerState) => {
  const { editable, editing, selected, isLastVisible, rowHeight, classes } =
    ownerState;
  const slots = {
    root: [
      "row",
...
>(function GridRow(props, refProp) {
  ...
  React.useLayoutEffect(() => {
    if (
      ...
      // Fallback for IE
      apiRef.current.unstable_storeRowHeightMeasurement(
        rowId,
  ...
  const getCell = React.useCallback(
    (
      ...
      const classNames = apiRef.current.unstable_applyPipeProcessors(
        "cellClassName",
        [],
        {
          id: rowId,
      ...
      if (editCellState == null && column.renderCell) {
        ...
        classNames.push(
          clsx(
            ...
            rootProps.classes?.["cell--withRenderer"]
          )
        );
      }
      ...
      if (editCellState != null && column.renderEditCell) {
        ...
        classNames.push(
          clsx(
            ...
            rootProps.classes?.["cell--editing"]
          )
        );
      }
      ...
      const isSelected = apiRef.current.unstable_applyPipeProcessors(
        "isCellSelected",
        false,
        {
          id: rowId,
      ...
      return (
        <rootProps.components.Cell
          ...
          async={cellParams.async}
          registerAPI={cellParams.registerAPI}
          crud={cellParams.crud}
          maxResult={cellParams.maxResult}
          typeGet={cellParams.typeGet}
          multiTypeGet={cellParams.multiTypeGet}
          idCountry={cellParams.idCountry}
          idState={cellParams.idState}
          acronym={cellParams.acronym}
          ...
          maxDate={cellParams.maxDate}
          minDate={cellParams.minDate}
          useFixedListToAsync={cellParams.useFixedListToAsync}
  ...
  if (sizes?.spacingTop) {
    const property =
      rootProps.rowSpacingType === "border" ? "borderTopWidth" : "marginTop";
  ...
  const rowClassNames = apiRef.current.unstable_applyPipeProcessors(
    "rowClassName",
    [],
    rowId
  );
  ...
  if (typeof rootProps.getRowClassName === "function") {
    const indexRelativeToCurrentPage =
      index - (currentPage.range?.firstRowIndex || 0);
