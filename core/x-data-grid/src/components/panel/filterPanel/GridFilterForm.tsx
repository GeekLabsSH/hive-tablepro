...
import { SelectChangeEvent } from "@cronoslogistics/hive-tablepro/core/mui-material/src/Select";
import {
...
const getLogicOperatorLocaleKey = (logicOperator: GridLogicOperator) => {
  switch (logicOperator) {
    ...
    default:
      throw new Error(
        "HIVE: Invalid `logicOperator` property in the `GridFilterPanel`."
      );
  }
};
...
const GridFilterForm = React.forwardRef<HTMLDivElement, GridFilterFormProps>(
  function GridFilterForm(props, ref) {
    ...
    const hasLogicOperatorColumn: boolean =
      hasMultipleFilters && logicOperators.length > 0;
    ...
    const filteredColumns = React.useMemo(() => {
      ...
      return filterableColumns.filter((column) =>
        filteredFields.includes(column.field)
      );
    ...
    //MuiFormControl-root MuiDataGrid-filterFormDeleteIcon css-rne967-MuiFormControl-root-MuiDataGrid-filterFormDeleteIcon
      ...
      >
        ...
        >
          <rootProps.components.BaseSelect
            ...
            disabled={
              !!disableMultiFilterOperator || logicOperators.length === 1
            }
            ...
            {logicOperators.map((logicOperator) => (
              ...
              >
                {apiRef.current.getLocaleText(
                  getLogicOperatorLocaleKey(logicOperator)
                )}
              </OptionComponent>
            ))}
          </rootProps.components.BaseSelect>
          ...
          >
            {currentColumn?.filterOperators?.map((operator) => (
              <OptionComponent key={operator.value} value={operator.value}>
                {operator.label ||
                  apiRef.current.getLocaleText(
                    `filterOperator${capitalize(
                      operator.value
                    )}` as "filterOperatorContains"
                  )}
              </OptionComponent>
            ))}
          </rootProps.components.BaseSelect>
