...
const defaultSearchPredicate: NonNullable<
  GridColumnsPanelProps["searchPredicate"]
> = (column, searchValue) => {
  return (
...
function GridColumnsPanel(props: GridColumnsPanelProps) {
  ...
  const currentColumns = React.useMemo(() => {
    ...
    return sortedColumns.filter((column) =>
      searchPredicate(column, searchValueToCheck)
    );
  ...
  React.useEffect(() => {
    if (autoFocusSearchField) {
      ...
      firstSwitchRef.current &&
      typeof firstSwitchRef.current.focus === "function"
    ) {
