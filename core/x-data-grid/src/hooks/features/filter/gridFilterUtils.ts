...
const filterModelDisableMultiColumnsFilteringWarning = buildWarning(
  [
    'HIVE: The `filterModel` can only contain a single item when the `disableMultipleColumnsFiltering` prop is set to `true`.',
...
const filterModelMissingItemIdWarning = buildWarning(
  'HIVE: The `id` field is required on `filterModel.items` when you use multiple filters.',
  'error',
);

const filterModelMissingItemOperatorWarning = buildWarning(
  'HIVE: The `operator` field is required on `filterModel.items`, one or more of your filtering item has no `operator` provided.',
  'error',
);
...
const getFilterCallbackFromItem = (
  ...
  if (!filterOperators?.length) {
    throw new Error(`HIVE: No filter operators found for column '${column.field}'.`);
  }
  ...
  if (!filterOperator) {
    throw new Error(
      `HIVE: No filter operator found for column '${column.field}' and operator value '${newFilterItem.operator}'.`,
    );
  }
