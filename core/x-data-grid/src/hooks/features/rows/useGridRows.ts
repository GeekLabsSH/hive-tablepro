...
export const useGridRows = (
  ...
  const lookup = React.useMemo(
    () =>
      currentPage.rows.reduce<Record<GridRowId, number>>(
        (acc, { id }, index) => {
          acc[id] = index;
          return acc;
        },
  ...
  const throttledRowsChange = React.useCallback(
    ({
      ...
      const throttleRemainingTimeMs =
        props.throttleRowsMs - (Date.now() - lastUpdateMs.current);
  ...
  const updateRows = React.useCallback<GridRowApi["updateRows"]>(
    (updates) => {
      if (props.signature === GridSignature.DataGrid && updates.length > 1) {
        throw new Error(
          [
            "HIVE: You can't update several rows at once in `apiRef.current.updateRows` on the DataGrid.",
  ...
  const setRowChildrenExpansion = React.useCallback<
    GridRowProApi["setRowChildrenExpansion"]
  >(
    (id, isExpanded) => {
      const currentNode = apiRef.current.getRowNode(id);
      if (!currentNode) {
        throw new Error(`HIVE: No row with id #${id} found`);
      }

      if (currentNode.type !== "group") {
        throw new Error("HIVE: Only group nodes can be expanded or collapsed");
      }

      const newNode: GridGroupNode = {
        ...currentNode,
  ...
  const getRowGroupChildren = React.useCallback<
    GridRowProApi["getRowGroupChildren"]
  >(
    ({
      ...
      if (applyFiltering) {
        ...
        children = children.filter(
          (childId) => filteredRowsLookup[childId] !== false
        );
      }
  ...
  const setRowIndex = React.useCallback<GridRowProApi["setRowIndex"]>(
    (rowId, targetIndex) => {
      ...
      if (!node) {
        throw new Error(`HIVE: No row with id #${rowId} found`);
      }

      if (node.parent !== GRID_ROOT_GROUP_ID) {
        throw new Error(
          `HIVE: The row reordering do not support reordering of grouped rows yet`
        );
      }

      if (node.type !== "leaf") {
        throw new Error(
          `HIVE: The row reordering do not support reordering of footer or grouping rows`
        );
      }
  ...
  const replaceRows = React.useCallback<GridRowApi["unstable_replaceRows"]>(
    (firstRowToRender, newRows) => {
      if (props.signature === GridSignature.DataGrid && newRows.length > 1) {
        throw new Error(
          [
            "HIVE: You can't replace rows using `apiRef.current.unstable_replaceRows` on the DataGrid.",
            "You need to upgrade to DataGridPro or DataGridPremium component to unlock this feature.",
          ].join("\n")
        );
      }
      ...
      const dataRowIdToIdLookup = {
        ...gridRowsDataRowIdToIdLookupSelector(apiRef),
      };
      ...
      const dataRowIds = rootGroupChildren.filter(
        (childId) => tree[childId].type === "leaf"
      );

      apiRef.current.caches.rows.dataRowIdToModelLookup =
  ...
  >(() => {
    ...
    if (
      apiRef.current.getActiveStrategy("rowTree") !==
    ...
    apiRef.current.setState((state) => {
      const response = apiRef.current.unstable_applyPipeProcessors(
        "hydrateRows",
        {
          ...
          treeDepths: gridRowTreeDepthsSelector(
            state,
            apiRef.current.instanceId
          ),
          dataRowIds: gridDataRowIdsSelector(state, apiRef.current.instanceId),
          dataRowIdToModelLookup: gridRowsLookupSelector(
            state,
            apiRef.current.instanceId
          ),
          dataRowIdToIdLookup: gridRowsDataRowIdToIdLookupSelector(
            state,
            apiRef.current.instanceId
          ),
        }
  ...
  React.useEffect(() => {
    ...
    const isNewLoadingAlreadyInState =
      apiRef.current.caches.rows.loadingPropBeforePartialUpdates ===
      props.loading;
    const isNewRowCountAlreadyInState =
      apiRef.current.caches.rows.rowCountPropBeforePartialUpdates ===
      props.rowCount;
