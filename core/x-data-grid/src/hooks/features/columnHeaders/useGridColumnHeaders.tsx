...
export const useGridColumnHeaders = (props: UseGridColumnHeadersProps) => {
  ...
  const [renderContext, setRenderContext] =
    React.useState<GridRenderContext | null>(null);
  const prevRenderContext = React.useRef<GridRenderContext | null>(
    renderContext
  );
  ...
  const getFirstColumnIndexToRenderRef = React.useRef<
    typeof getFirstColumnIndexToRender
  >(
  ...
  const getColumnHeaders = (params?: GetHeadersParams, other = {}) => {
    ...
    for (let i = 0; i < renderedColumns.length; i += 1) {
      ...
      const tabIndex =
        (columnHeaderTabIndexState !== null &&
          columnHeaderTabIndexState.field === colDef.field) ||
        (isFirstColumn && !hasTabbableElement)
          ? 0
          : -1;
      const hasFocus =
        columnHeaderFocus !== null && columnHeaderFocus.field === colDef.field;
      const open =
        columnMenuState.open && columnMenuState.field === colDef.field;
      ...
      columns.push(
        <GridColumnHeaderItem
          ...
          filterItemsCounter={
            ...
            filterColumnLookup[colDef.field].length
          }
  ...
  const getColumnGroupHeaders = (params?: GetHeadersParams) => {
    ...
    if (
      ...
      columnsToRender.renderedColumns.length === 0
    ) {
      return null;
    }
    ...
    for (let depth = 0; depth < headerGroupingMaxDepth; depth += 1) {
      ...
      const firstColumnFieldToRender =
        visibleColumns[firstColumnToRender].field;
      const firstGroupToRender =
        apiRef.current.unstable_getColumnGroupPath(firstColumnFieldToRender)[
          depth
        ] ?? null;

      const firstGroupIndex = rowStructure.findIndex(
        ({ groupId, columnFields }) =>
          groupId === firstGroupToRender &&
          columnFields.includes(firstColumnFieldToRender)
      );

      const lastColumnFieldToRender =
        visibleColumns[lastColumnToRender - 1].field;
      const lastGroupToRender =
        apiRef.current.unstable_getColumnGroupPath(lastColumnFieldToRender)[
          depth
        ] ?? null;
      const lastGroupIndex = rowStructure.findIndex(
        ({ groupId, columnFields }) =>
          groupId === lastGroupToRender &&
          columnFields.includes(lastColumnFieldToRender)
      );
      ...
      const leftOverflow = visibleColumnGroupHeader[0].columnFields.indexOf(
        firstColumnFieldToRender
      );
      ...
      columns.push(
        ...
        >
          {depthInfo.elements.map(
            (
              { groupId, width, fields, colIndex, hasFocus, tabIndex },
              groupIndex
            ) => {
              return (
                <GridColumnGroupHeader
                  ...
                  depth={depthIndex}
                  isLastColumn={
                    colIndex === visibleColumns.length - fields.length
                  }
