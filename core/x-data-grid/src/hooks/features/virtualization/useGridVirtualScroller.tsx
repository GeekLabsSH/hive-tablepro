...
export const useGridVirtualScroller = (props: UseGridVirtualScrollerProps) => {
  ...
  const getNearestIndexToRender = React.useCallback(
    (offset: number) => {
      ...
      if (
        allRowsMeasured ||
        rowsMeta.positions[lastMeasuredIndexRelativeToCurrentPage] >= offset
      ) {
  ...
  const computeRenderContext = React.useCallback(() => {
    ...
    for (
      ...
      i < lastRowToRender && !hasRowWithAutoHeight;
      i += 1
    ) {
