...
import {
  GridEditingSharedApi, GridEditingSharedPrivateApi, GridRowEditingApi, GridRowEditingPrivateApi, GridRowModesModel,
  GridRowModesModelProps, GridStartRowEditModeParams, GridStopRowEditModeParams
} from '../../../models/api/gridEditingApi';
import { GridEventListener } from '../../../models/events/gridEventListener';
import {
  GridEditCellProps, GridEditingState, GridEditModes, GridEditRowProps, GridRowModes
} from '../../../models/gridEditRowModel';
import { GridRowId } from '../../../models/gridRows';
import { GridCellParams } from '../../../models/params/gridCellParams';
import {
  GridRowEditStartParams, GridRowEditStartReasons, GridRowEditStopParams, GridRowEditStopReasons
...
const missingOnProcessRowUpdateErrorWarning = buildWarning(
  [
    'HIVE: A call to `processRowUpdate` threw an error which was not handled because `onProcessRowUpdateError` is missing.',
  ...
  const throwIfNotEditable = React.useCallback(
    (id: GridRowId, field: string) => {
      ...
      if (!apiRef.current.isCellEditable(params)) {
        throw new Error(`HIVE: The cell with id=${id} and field=${field} is not editable.`);
      }
    },
    [apiRef],
  );
  ...
  const throwIfNotInMode = React.useCallback(
    (id: GridRowId, mode: GridRowModes) => {
      if (apiRef.current.getRowMode(id) !== mode) {
        throw new Error(`HIVE: The row with id=${id} is not in ${mode} mode.`);
      }
    },
    [apiRef],
  );
