...
import {
  GridRowMultiSelectionApi, GridRowSelectionApi
...
> = (state, props) => ({
  ...state,
  rowSelection: props.rowSelection ? getSelectionModelPropValue(props.rowSelectionModel) ?? [] as any : [],
});
...
export const useGridRowSelection = (
  ...
  const propRowSelectionModel: any = React.useMemo(() => {
  ...
  const setRowSelectionModel = React.useCallback<GridRowSelectionApi['setRowSelectionModel']>(
    (model) => {
      if (
        ...
        throw new Error(
          [
            'HIVE: `rowSelectionModel` can only contain 1 item in DataGrid.',
  ...
  const handleGroupingSelectionCheckboxChange = React.useCallback<
    GridEventListener<'groupingSelectionCheckboxChange'>
  >(
    (params) => {
      apiRef.current.selectRows(params.listId, params.value);
    },
    [apiRef],
  );
  ...
  useGridApiEventHandler(
    apiRef,
    'groupingSelectionCheckboxChange',
    handleGroupingSelectionCheckboxChange,
  );
  useGridApiEventHandler(
    apiRef,
    'cellMouseDown',
