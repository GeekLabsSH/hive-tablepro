...
export const useGridPrintExport = (
  ...
  const previousGridState = React.useRef<GridInitialStateCommunity | null>(
    null
  );
  ...
  const buildPrintWindow = React.useCallback(
    (title?: string): HTMLIFrameElement => {
  ...
  const handlePrintWindowLoad: PrintWindowOnLoad = React.useCallback(
    (printWindow, options): void => {
      ...
      const gridMain: HTMLElement | null = gridClone.querySelector(
        `.${gridClasses.main}`
      );
      ...
      if (normalizeOptions.copyStyles) {
        const headStyleElements = doc.current!.querySelectorAll(
  ...
  const exportDataAsPrint = React.useCallback<
    GridPrintExportApi["exportDataAsPrint"]
  >(
    async (options) => {
      logger.debug(`Export data as Print`);

      if (!apiRef.current.rootElementRef!.current) {
        throw new Error("HIVE: No grid root element available.");
      }
