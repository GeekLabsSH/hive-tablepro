...
export type MuiPickersAdapterContextNullableValue<TDate> = {
  [K in keyof MuiPickersAdapterContextValue<TDate>]:
    | MuiPickersAdapterContextValue<TDate>[K]
    | null;
};
 ...
 */
export function LocalizationProvider<TDate>(
  inProps: LocalizationProviderProps<TDate>
) {
    ...
    if (!adapter.isMUIAdapter) {
      throw new Error(
        [
          ...
          "For example, `import { AdapterDayjs } from '@cronoslogistics/hive-tablepro/core/x-date-pickers/src/AdapterDayjs'` instead of `import AdapterDayjs from '@date-io/dayjs'`",
  ...
  const contextValue: MuiPickersAdapterContextNullableValue<TDate> =
    React.useMemo(() => {
      return {
        utils,
        defaultDates,
