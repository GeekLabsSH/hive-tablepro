...
import { useLocaleText, useNow, useUtils } from "../internals/hooks/useUtils";
...
const useUtilityClasses = (ownerState: DayCalendarProps<any>) => {
  ...
  return compClasses(slots, getDayCalendarUtilityClass, classes);
};
...
function WrappedDay<TDate extends unknown>({
  ...
  "onFocus" | "onBlur" | "onKeyDown" | "onDaySelect"
> & {
...
export function DayCalendar<TDate>(inProps: DayCalendarProps<TDate>) {
  ...
  const handleFocus = useEventCallback(
    (event: React.FocusEvent<HTMLButtonElement>, day: TDate) => focusDay(day)
  );
  const handleBlur = useEventCallback(
    (event: React.FocusEvent<HTMLButtonElement>, day: TDate) => {
