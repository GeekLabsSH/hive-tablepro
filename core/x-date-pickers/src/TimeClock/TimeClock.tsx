...
const useUtilityClasses = (ownerState: TimeClockProps<any>) => {
  ...
  return compClasses(slots, getTimeClockUtilityClass, classes);
};
...
>(inProps: TimeClockProps<TDate>, ref: React.Ref<HTMLDivElement>) {
          ...
          switch (viewType) {
            case "hours":
              return !shouldDisableTime(
                utils.setHours(selectedTimeOrMidnight, timeValue),
                "hours"
              );
            case "minutes":
              return !shouldDisableTime(
...
TimeClock.propTypes = {
  ...
  views: PropTypes.arrayOf(
    PropTypes.oneOf(["hours", "minutes", "seconds"]).isRequired
  ),
} as any;
