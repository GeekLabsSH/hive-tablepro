...
const useUtilityClasses = (ownerState: ClockNumberProps) => {
  ...
  return compClasses(slots, getClockNumberUtilityClass, classes);
};
...
export function ClockNumber(inProps: ClockNumberProps) {
  ...
  const { className, disabled, index, inner, label, selected, ...other } =
    props;
  ...
  const length =
    ((CLOCK_WIDTH - CLOCK_HOUR_WIDTH - 2) / 2) * (inner ? 0.65 : 1);
    ...
    <ClockNumberRoot
      ...
      style={{
        transform: `translate(${x}px, ${
          y + (CLOCK_WIDTH - CLOCK_HOUR_WIDTH) / 2
        }px`,
      }}
