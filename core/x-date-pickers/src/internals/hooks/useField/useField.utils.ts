...
import {
  AvailableAdjustKeyCode, FieldSection, FieldSectionsValueBoundaries, FieldValueType, SectionNeighbors,
  SectionOrdering
} from './useField.types';
...
export const getDateSectionConfigFromFormatToken = <TDate>(
  ...
  if (config == null) {
    throw new Error(
      [
        `HIVE: The token "${formatToken}" is not supported by the Date and Time Pickers.`,
...
export const validateSections = <TSection extends FieldSection>(
  ...
  if (process.env.NODE_ENV !== null) {
    if (!warnedOnceInvalidSection) {
      ...
      if (invalidSection) {
        console.warn(
          `HIVE: The field component you are using is not compatible with the "${invalidSection.dateSectionName} date section.`,
