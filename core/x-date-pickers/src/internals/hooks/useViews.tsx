import { unstable_useControlled as useControlled } from "@cronoslogistics/hive-tablepro/core/mui-utils/src";
...
export interface ExportedUseViewsOptions<TView extends DateOrTimeView>
  extends MakeOptional<
    Omit<UseViewsOptions<any, TView>, "onChange">,
    "openTo" | "views"
  > {}
...
export function useViews<TValue, TView extends DateOrTimeView>({
  ...
  if (process.env.NODE_ENV !== null) {
    if (!warnedOnceNotValidView) {
      if (inView != null && !views.includes(inView)) {
        console.warn(
          `HIVE: \`view="${inView}"\` is not a valid prop.`,
      ...
      if (inView == null && openTo != null && !views.includes(openTo)) {
        console.warn(
          `HIVE: \`openTo="${openTo}"\` is not a valid prop.`,
          `It must be an element of \`views=["${views.join('", "')}"]\`.`
        );
        warnedOnceNotValidView = true;
      }
  ...
  const defaultView = React.useRef(
  ...
  const setValueAndGoToNextView = useEventCallback(
    (value: TValue, currentViewSelectionState?: PickerSelectionState) => {
      const isSelectionFinishedOnCurrentView =
        currentViewSelectionState === "finish";
  ...
  const handleFocusedViewChange = useEventCallback(
    (viewToFocus: TView, hasFocus: boolean) => {
      if (hasFocus) {
