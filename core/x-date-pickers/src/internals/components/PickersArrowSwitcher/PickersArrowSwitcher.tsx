...
const useUtilityClasses = (ownerState: PickersArrowSwitcherOwnerState) => {
  ...
  return compClasses(slots, getPickersArrowSwitcherUtilityClass, classes);
};

export const PickersArrowSwitcher = React.forwardRef(
  function PickersArrowSwitcher(
    ...
    const NextIconButton =
      slots?.nextIconButton ??
      components?.NextIconButton ??
      PickersArrowSwitcherButton;
    const nextIconButtonProps = useSlotProps({
      elementType: NextIconButton,
      externalSlotProps:
        slotProps?.nextIconButton ?? componentsProps?.nextIconButton,
      additionalProps: {
        size: "small",
    ...
    const LeftArrowIcon =
      slots?.leftArrowIcon ?? components?.LeftArrowIcon ?? ArrowLeft;
    // The spread is here to avoid this bug mui/material-ui#34056
    const { ownerState: leftArrowIconOwnerState, ...leftArrowIconProps } =
      useSlotProps({
        elementType: LeftArrowIcon,
        externalSlotProps:
          slotProps?.leftArrowIcon ?? componentsProps?.leftArrowIcon,
        ownerState: undefined,
      });

    const RightArrowIcon =
      slots?.rightArrowIcon ?? components?.RightArrowIcon ?? ArrowRight;
    // The spread is here to avoid this bug mui/material-ui#34056
    const { ownerState: rightArrowIconOwnerState, ...rightArrowIconProps } =
      useSlotProps({
        elementType: RightArrowIcon,
        externalSlotProps:
          slotProps?.rightArrowIcon ?? componentsProps?.rightArrowIcon,
        ownerState: undefined,
      });
