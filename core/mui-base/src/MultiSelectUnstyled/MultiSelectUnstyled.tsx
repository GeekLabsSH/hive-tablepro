...
function defaultRenderMultipleValues<TValue>(
...
function useUtilityClasses(ownerState: MultiSelectUnstyledOwnerState<any>) {
  ...
  return compClasses(slots, getSelectUnstyledUtilityClass, {});
}
 ...
 */
const MultiSelectUnstyled = React.forwardRef(function MultiSelectUnstyled<
  TValue extends {}
>(
  ...
  const buttonProps: WithOptionalOwnerState<
    MultiSelectUnstyledRootSlotProps<TValue>
  > = useSlotProps({
    elementType: Button,
  ...
  const listboxProps: WithOptionalOwnerState<
    MultiSelectUnstyledListboxSlotProps<TValue>
  > = useSlotProps({
