    ...
    <GridContextProvider privateApiRef={privateApiRef} props={props}>
      ...
      >
        ...
        <GridBody
          ...
          virtualScrollerProps={{ id: props.id }}
        />
        <GridFooterPlaceholder />
      </GridRoot>
    </GridContextProvider>
...
DataGridProRaw.propTypes = {
  ...
  checkboxSelectionVisibleOnly: chainPropTypes(PropTypes.bool, (props: any) => {
    if (!props.pagination && props.checkboxSelectionVisibleOnly) {
      return new Error(
        "HIVE: The `checkboxSelectionVisibleOnly` prop has no effect when the pagination is not enabled."
      );
    }
    return null;
  }),
   ...
   * @default "client"
   */
  filterMode: chainPropTypes(
    PropTypes.oneOf(["client", "server"]),
    (props: any) => {
      if (props.treeData && props.filterMode === "server") {
        return new Error(
          'HIVE: The `filterMode="server"` prop is not available when the `treeData` is enabled.'
        );
      }
      return null;
    }
  ...
  hideFooterRowCount: chainPropTypes(PropTypes.bool, (props: any) => {
    if (props.pagination && props.hideFooterRowCount) {
      return new Error(
        "HIVE: The `hideFooterRowCount` prop has no effect when the pagination is enabled."
      );
    }
    return null;
  }),
