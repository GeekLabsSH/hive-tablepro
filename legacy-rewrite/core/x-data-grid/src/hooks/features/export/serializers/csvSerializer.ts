...
const objectFormattedValueWarning = buildWarning([
  'HIVE: When the value of a field is an object or a `renderCell` is provided, the CSV export might not display the value correctly.',
...
export function buildCSV(options: BuildCSVOptions): string {
  ...
  const CSVBody = rowIds
    .reduce<string>(
      (acc, id) =>
        `${acc}${serializeRow(id, columns, getCellParams, ';').join(
          ';',
        )}\r\n`,
      '',
    )
  ...
  const CSVHead = `${columns
    ...
    .map((column) => serializeCellValue(column.headerName || column.field, ';'))
    .join(';')}\r\n`;
