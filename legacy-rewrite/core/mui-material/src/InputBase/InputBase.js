...
const useUtilityClasses = (ownerState) => {
  ...
  return compClasses(slots, getInputBaseUtilityClass, classes);
};
...
const InputBase = React.forwardRef(function InputBase(inProps, ref) {
  ...
  const handleInputRefWarning = React.useCallback((instance) => {
    if (process.env.NODE_ENV !== null) {
      if (instance && instance.nodeName !== "INPUT" && !instance.focus) {
        console.error(
          [
            "HIVE: You have provided a `inputComponent` to the input component",
  ...
  const handleChange = (event, ...args) => {
    if (type == "date" && maxDate != undefined) {
      const currentDate = new Date();
      const validMaxDate = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate() + maxDate);

      const dataString = event?.target?.value;
      const partesData = dataString.split("-");
      const year = parseInt(partesData[0]);
      const month = parseInt(partesData[1]) - 1;
      const day = parseInt(partesData[2]);

      const checkedDate = new Date(year, month, day);
      
      if (checkedDate > validMaxDate) {
        return checkDirty({
          value: null,
        });
      }
  ...
  if (multiline && InputComponent === "input") {
    if (rows) {
      if (process.env.NODE_ENV !== null) {
        if (minRows || maxRows) {
          console.warn(
            "HIVE: You can not use the `minRows` or `maxRows` props when the input `rows` prop is set."
          );
        }
      }
      inputProps = {
  ...
  function handleVerifyMaxDate (max) {
    var y = new Date().getFullYear();
    var m = new Date().getMonth() + 1;
    var d = new Date().getDate() + max;
  ...
  function handleVerifyMinDate (min) {
    var y = new Date().getFullYear();
    var m = new Date().getMonth() + 1;
    var d = new Date().getDate() - min;
      ...
      >
        ...
        <FormControlContext.Provider value={null}>
          <Input
            ...
            onFocus={handleFocus}
            min={type == "date" && minDate != undefined ? handleVerifyMinDate(minDate) : null}
            max={type == "date" && maxDate != undefined ? handleVerifyMaxDate(maxDate) : null}
          />
