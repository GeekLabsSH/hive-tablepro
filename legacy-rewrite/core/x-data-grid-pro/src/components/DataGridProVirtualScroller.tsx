...
})<{ ownerState: VirtualScrollerPinnedColumnsProps }>(
  ({ theme, ownerState }) => {
...
interface DataGridProVirtualScrollerProps
...
>(function DataGridProVirtualScroller(props, ref) {
  ...
  const apiRef = useGridPrivateApiContext() as any;
  ...
  const expandedRowIds = useGridSelector(
    apiRef,
    gridDetailPanelExpandedRowIdsSelector as any
  ) as any;
  const detailPanelsContent = useGridSelector(
    apiRef,
    gridDetailPanelExpandedRowsContentCacheSelector as any
  ) as any;
  const detailPanelsHeights = useGridSelector(
    apiRef,
    gridDetailPanelExpandedRowsHeightCacheSelector as any
  ) as any;
  ...
  const pinnedColumns = useGridSelector(
    apiRef,
    gridPinnedColumnsSelector as any
  );
  ...
  const getDetailPanels = () => {
    ...
    for (let i = 0; i < uniqueExpandedRowIds.length; i += 1) {
      ...
      if (React.isValidElement(content as any) && exists) {
    ...
    <GridVirtualScroller {...getRootProps(other)}>
      ...
      {topPinnedRowsData.length > 0 ? (
        <VirtualScrollerPinnedRows
          ...
          ownerState={{ position: "top" } as any}
          style={{
      ...
      {bottomPinnedRowsData.length > 0 ? (
        <VirtualScrollerPinnedRows
          ...
          ownerState={{ position: "bottom" } as any}
          style={{
            width: contentProps.style.width,
