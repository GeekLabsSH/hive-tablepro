...
export const useGridColumnPinning = (
  ...
  const updateHoveredClassOnSiblingRows = React.useCallback(
    (event: React.MouseEvent<HTMLElement>) => {
      ...
      const rowElements =
        apiRef.current.virtualScrollerRef!.current!.querySelectorAll(
  ...
  const handleMouseEnter = React.useCallback<
    GridEventListener<"rowMouseEnter">
  >(
    (params, event) => {
  ...
  const handleMouseLeave = React.useCallback<
    GridEventListener<"rowMouseLeave">
  >(
    (params, event) => {
      updateHoveredClassOnSiblingRows(event);
    },
  ...
  const checkIfEnabled = React.useCallback(
    (methodName: keyof GridColumnPinningApi) => {
      if (props.disableColumnPinning) {
        throw new Error(
          `HIVE: You cannot call \`apiRef.current.${methodName}\` when \`disableColumnPinning\` is true.`
        );
      }
    },
    [props.disableColumnPinning]
